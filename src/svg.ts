...
function unsafeSvg(message = "SVG output contains active or external content."): never {
...
function parseTag(raw: string): ParsedTag {
  ...
  while (cursor < body.length) {
    ...
    if (!hadWhitespace) unsafeSvg("SVG attributes must be whitespace-separated.");
    ...
    while (/\s/.test(body[cursor] ?? "")) cursor += 1;
    if (body[cursor] !== "=") unsafeSvg("SVG attributes must have quoted values.");
    cursor += 1;
    while (/\s/.test(body[cursor] ?? "")) cursor += 1;
    ...
    if (cursor >= body.length) unsafeSvg("SVG output contains an unterminated attribute.");
    const value = body.slice(valueStart, cursor);
    cursor += 1;
    const normalizedName = attributeName.toLowerCase();
    if (names.has(normalizedName)) unsafeSvg("SVG output contains duplicate attributes.");
...
function validateSvgMarkup(svg: string): void {
  ...
  while (cursor < svg.length) {
    ...
    if (tagStart === -1) {
      if (closedRoot && svg.slice(cursor).trim()) unsafeSvg("SVG output has trailing content.");
      break;
    }
    if ((stack.length === 0 || closedRoot) && svg.slice(cursor, tagStart).trim()) {
    ...
    const localName = normalizedName.split(":").at(-1) ?? normalizedName;
    if (tag.closing) {
      if (stack.pop() !== normalizedName) unsafeSvg("SVG element nesting is invalid.");
      if (stack.length === 0) closedRoot = true;
    } else {
      if (closedRoot) unsafeSvg("SVG output has more than one root element.");
      if (!sawRoot) {
        if (normalizedName !== "svg") unsafeSvg("Vectorizer output is not an SVG document.");
        sawRoot = true;
      }
...
export function svgAssetToText(asset: IconBinaryAsset): string {
  if (asset.mimeType.split(";", 1)[0]?.trim().toLowerCase() !== "image/svg+xml") {
