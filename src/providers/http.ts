  ...
  try {
    ...
    const readNext = () =>
      new Promise<ReadableStreamReadResult<Uint8Array>>((resolve, reject) => {
        ...
        reader.read().then(resolveOnce, (error) =>
    ...
    const body = new ReadableStream<Uint8Array>({
      async pull(output) {
        try {
          ...
          settle();
          if (controller.signal.aborted) void reader.cancel(error).catch(() => undefined);
          output.error(error);
        }
      },
      async cancel(reason) {
...
async function readTextWithinLimit(
  ...
  if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
    throw new IconForgeError("PROVIDER_ERROR", `${label} response exceeded the byte limit.`);
  }
...
export async function readBytesWithinLimit(
  ...
  const declaredLength = Number(response.headers.get("content-length") ?? "0");
  if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
    throw new IconForgeError("PROVIDER_ERROR", `${label} exceeded the byte limit.`);
  }
...
export function assertOpenRouterModelId(value: string): string {
  ...
  if (
    ...
    throw new IconForgeError("INVALID_INPUT", "OpenRouter model id is invalid.");
  }
  return model;
}
